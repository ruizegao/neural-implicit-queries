...
def save_render_current_view(args, implicit_func, params, cast_frustum, opts, matcaps, surf_color):
    ...
    img, depth, count, _, eval_sum, raycast_time = render.render_image(implicit_func, params, root, look, up, left, res,
                                                                       fov_deg, cast_frustum, opts,
...
def do_sample_surface(opts, implicit_func, params, n_samples, sample_width, n_node_thresh, do_viz_tree,
                      do_uniform_sample):
    ...
    with Timer("sample points"):
        sample_points = sample_surface(implicit_func, params, lower, upper, n_samples, sample_width, rngkey,
    ...
    if do_viz_tree:
        out_dict = construct_uniform_unknown_levelset_tree(implicit_func, params, lower, upper, n_node_thresh,
                                                           offset=sample_width)
...
def do_hierarchical_mc(opts, implicit_func, params, n_mc_depth, do_viz_tree, compute_dense_cost):
    ...
    if do_viz_tree:
        n_mc_subcell = 3
        out_dict = construct_uniform_unknown_levelset_tree(implicit_func, params, lower, upper,
    ...
    def callback():
        ...
        if psim.TreeNode("Eval options"):
            ...
            if mode == 'affine_truncate':
                ...
                changed, affine_opts['affine_n_truncate'] = psim.InputInt("affine_n_truncate",
                                                                          affine_opts['affine_n_truncate'])
                if changed:
                    implicit_func, params = implicit_mlp_utils.generate_implicit_from_file(args.input, mode=mode,
                                                                                           **affine_opts)

                changed, affine_opts['affine_truncate_policy'] = utils.combo_string_picker("Method", affine_opts[
                    'affine_truncate_policy'], truncate_policies)
                if changed:
                    implicit_func, params = implicit_mlp_utils.generate_implicit_from_file(args.input, mode=mode,
                                                                                           **affine_opts)
            ...
            if mode == 'affine_append':
                ...
                changed, affine_opts['affine_n_append'] = psim.InputInt("affine_n_append",
                                                                        affine_opts['affine_n_append'])
                if changed:
                    implicit_func, params = implicit_mlp_utils.generate_implicit_from_file(args.input, mode=mode,
                                                                                           **affine_opts)
        ...
        if psim.TreeNode("Sample Surface "):
            ...
            if psim.Button("Sample"):
                do_sample_surface(opts, implicit_func, params, n_sample_pts, sample_width, n_node_thresh, do_viz_tree,
                                  do_uniform_sample)
