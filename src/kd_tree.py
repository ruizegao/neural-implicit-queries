...
def construct_full_uniform_unknown_levelset_tree_iter(
    ...
    N_out = 2 * N_in
    d = node_lower.shape[-1]
    internal_node_mask = jnp.logical_not(jnp.isnan(node_lower[:, 0]))
    node_types = jnp.full((N_in,), jnp.nan)
    node_split_dim = jnp.full((N_in,), jnp.nan, dtype=jnp.int32)
    node_split_val = jnp.full((N_in,), jnp.nan)
    node_lower_out = jnp.full((N_out, 3), jnp.nan)
    node_upper_out = jnp.full((N_out, 3), jnp.nan)
    ...
    eval_func = jax.vmap(eval_one_node)

    if batch_size is None:
        internal_node_types, internal_node_split_dim = eval_func(
            node_lower[internal_node_mask], node_upper[internal_node_mask])
        node_types = node_types.at[internal_node_mask].set(internal_node_types)
        node_split_dim = node_split_dim.at[internal_node_mask].set(internal_node_split_dim)
    ...
    # split the unknown nodes to children
    # (if split_children is False this will just not create any children at all)
    split_mask = jnp.logical_and(internal_node_mask, node_types == SIGN_UNKNOWN)
    ## now actually build the child nodes
    if continue_splitting:
        # extents of the new child nodes along each split dimension
        new_lower = node_lower
        ...
        # concatenate the new children to form output arrays
        inter_split_mask = jnp.repeat(split_mask, repeats=2)
        node_lower_out = node_lower_out.at[inter_split_mask].set(jnp.vstack(
            (newA_lower[split_mask], newB_lower[split_mask])))
        node_upper_out = node_upper_out.at[inter_split_mask].set(jnp.vstack(
            (newA_upper[split_mask], newB_upper[split_mask])))
        masked_node_split_val = new_mid[jnp.arange(len(node_types))[split_mask], node_split_dim[split_mask]]
        node_split_val = node_split_val.at[split_mask].set(masked_node_split_val)
...
def construct_full_uniform_unknown_levelset_tree(
    ...
    for i_split in range(n_splits):
        ...
        N_curr_nodes = jnp.logical_not(jnp.isnan(lower[:, 0])).sum()

        N_total_nodes += N_curr_nodes
        if quit_next:
            break

    node_lower = jnp.concatenate(node_lower)
    node_upper = jnp.concatenate(node_upper)
    node_type = jnp.concatenate(node_type)
    split_dim = jnp.concatenate(split_dim)
    split_val = jnp.concatenate(split_val)
...
def sample_surface(func, params, lower, upper, n_samples, width, rngkey, n_node_thresh=4096):
    ...
    n_samples_per_round = min(100 * n_samples, 100000)  # enough that we usually finish in one round
    ...
    print(f'Elapsed time in ms: {elapsed_time * 1000:.3f}')
...
@partial(jax.jit, static_argnames=("func", "n_samples_per_round"), donate_argnums=(8,))
...
def hierarchical_marching_cubes(func, params, lower, upper, depth, n_subcell_depth=2,
...
def closest_point_iter(func, params,
    ...
    def process_one(valid, query_id, lower, upper, query_loc, query_min_dist):
        ...
        node_width = jnp.max(upper - lower)
        node_center = 0.5 * (lower + upper)
        node_center_dist_offset = jnp.sqrt(
            jnp.sum(jnp.square(upper - lower)))  # maximum distance from the center to any point in the node
        ...
        needs_subdiv = utils.logical_and_all(
            (valid, ~is_outside, ~is_small, min_dist_to_point_in_node < query_min_dist))
    ...
    batch_needs_subdiv, batch_this_closest_point_dist, batch_node_center, batch_node_split_dim = \
        jax.vmap(process_one)(batch_valid, batch_query_id, batch_node_lower, batch_node_upper, batch_query_loc,
                              batch_query_min_dist)
